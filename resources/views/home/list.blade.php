...
@section('info')
    <title>{{$field->cate_name}}-{{$field->cate_title}}</title>
...
<div class="newblog left">
    @foreach($data as $d)
   <h2>{{$d->art_title}}</h2>
   <p class="dateview"><span>发布时间：{{date('Y-h-m',$d->art_time)}}</span><span>作者：{{$d->art_editor}}</span><span>查看次数：{{$d->art_view}}</span></p>
...
<aside class="right">
    ...
    <div class="rnav">
      <ul>
       ...
       <li class="rnav{{$k+1}}"><a href="{{url('cate/'.$c->cate_id)}}" target="_blank">{{$c->cate_name}}</a></li>
          @endforeach
     </ul>      
    </div>
    @endif
